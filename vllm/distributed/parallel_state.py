...
def get_inner_dp_world_group_k(k: int) -> GroupCoordinator:
    """返回共享边侧进程中第 ``k`` 个虚拟 DP 的 inner-DP 通信组。

    修改原因：同一物理边侧 rank 同时属于多个逻辑 DP 的边云通信链路，不能只
    保存一个全局 ``_INNER_DP_WORLD``。
    修改内容：按组内虚拟 DP rank 保存并返回各自的 edge+cloud coordinator。
    """
...
def init_distributed_environment(
    ...
    if (
        ...
        if config.parallel_config.is_shared_model_edge:
            # 修改原因：共享拓扑中 global DP rank 与 torch.distributed 物理 rank
            # 不再一一对应。
            # 修改内容：边侧映射到所属 edge group 的物理 rank；云侧排在全部
            # edge group 之后，并按全局 DP rank 计算偏移。
    ...
    if not torch.distributed.is_initialized():
        # 修改内容：初始化日志增加 edge_group_count，便于核对物理组与逻辑 DP
        # 的实际建组结果。
        logger.info(
            "world_size=%d rank=%d local_rank=%d distributed_init_method=%s backend=%s",
            world_size,
    ...
    if config is not None and config.parallel_config.nnodes_within_dp > 1:
        if parallel_config.data_parallel_size > 1:
            if config.parallel_config.is_shared_model_edge:
                # 修改原因：每个逻辑 DP 都需要独立的 edge-cloud inner group，
                # 但多个 group 允许复用同一个物理边侧 rank。
                # 修改内容：为每个 DP 构造 [物理 edge rank, 对应 cloud ranks]。
                ...
                # 修改原因：torch.distributed.new_group 对 default world 是
                # collective；若各 rank 只创建自己所属的组，调用顺序不同会死锁。
                # 修改内容：每个逻辑 DP 都构造“目标组 + 其余 rank 补集”的全
                # world 分区，让所有进程以相同顺序调用 new_group；只保存本进程
                # 实际所属的目标 coordinator。
                ...
                for dp_rank, target_ranks in enumerate(group_ranks_full):
                    ...
                    coordinator = init_model_parallel_group(
                        ...
                        backend,
                        # 修改原因：补集组只用于保持 collective 顺序，不承担 RPC。
                        # 修改内容：关闭其未使用的 MessageQueue broadcaster。
                ...
                if config.parallel_config.is_edge_node:
                    # 修改原因：边侧物理 rank 必须恰好拿到本组每个虚拟 DP 的
                    # coordinator，否则后续按 local DP 下标路由会越界或串组。
                    # 修改内容：校验 coordinator 数量并按 DP 顺序保存列表。
                    expected_group_size = (
                    ...
                    # 修改原因：每个云侧 rank 只能服务一个逻辑 DP。
                    # 修改内容：校验并保存该云 rank 唯一加入的 inner-DP 组。
...
def initialize_model_parallel(
    ...
    # 修改原因：共享边侧布局的物理 rank 数和标准 TP×PP×DP 公式不同，必须
    # 单独构造所有模型并行通信组。
    # 修改内容：每个 edge group 使用一个物理 rank，并承载固定子集的虚拟 DP。
    if parallel_config.is_shared_model_edge:
        ...
        # 修改原因：rank 布局错误会在 HCCL send/recv 阶段表现为超时。
        # 修改内容：建组前按“edge groups + 全部 cloud ranks”校验 world size。
        ...
        # 修改原因：边侧共享模型不做物理 TP，云侧仍需按每个 DP 组成 TP。
        # 修改内容：每个 edge rank 建单元素 TP 组；每个 DP 的 cloud ranks
        # 建一个 TP 组。
        ...
        # 修改原因：一个共享 edge rank 需要分别与本组多个 DP 的 cloud 首 rank
        # 通信，而其余 cloud TP rank 不直接参与跨边云 PP。
        # 修改内容：每个 edge group 建 [edge, hosted DP cloud-first...] PP 组，
        # 其余 cloud rank 建单元素 PP 组。
        ...
        # 修改原因：当前共享边云方案未在物理 rank 间启用 DCP/PCP。
        # 修改内容：为每个 rank 建立单元素 DCP/PCP 组，保持接口完整。
        ...
        # 修改原因：边侧 DP 维度是物理共享副本，云侧 DP 维度是相同 TP 位置。
        # 修改内容：全部 edge replicas 组成一个 DP 组；各 cloud TP 位置跨
        # 逻辑 DP 分别组成 DP 组。
        ...
        # 修改原因：边侧与云侧模型段的专家集合和通信域不同。
        # 修改内容：物理 edge replicas 与全部 cloud ranks 分别组成 EP 组。
        ...
        logger.info(
            ...
            is_edge,
            cloud_npu_count,
            # 修改内容：日志新增物理 edge group 数，便于核对共享拓扑。
            edge_group_count,
            data_parallel_size,
            world_size,
            tp_groups,
...
def in_the_same_node_as_edge_cloud(
    ...
    修改原因：原先假定只有一个共享 edge rank，无法判断 grouped_shared 的
    多个物理 edge replicas。
    修改内容：共享拓扑统一认为 ``[0, edge_shared_group_count)`` 为边侧，
    后续 rank 为云侧。
    """
    ...
    if is_shared:
        # 修改原因：共享拓扑的 edge rank 上界是物理共享组数，而不是已经归一化
        # 的每 DP edge_npu_count。
        # 修改内容：以 edge_shared_group_count 判断 rank 所属节点侧。
